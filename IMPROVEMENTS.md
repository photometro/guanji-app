...
- 2026-08-19：新增第 162 条【Bug】（“我的”页分段滑块背景层级高于 tab 内容，导致 tab 文案/图标被滑块遮挡；需统一滑块与内容层级并禁止滑块拦截点击），仅记录
- 2026-08-20：**#161/#162 实施完成（v3.33）**：统一“我的”页 row-btn 的导航语义与右箭头表达；修复 seg/tab 滑块、玻璃材质层和 tab 内容的层级及点击穿透；浏览器、APK 与真机回归结果见实施记录
- 2026-08-20：**#163 实施完成（v3.34）**：重组“我的”页 6 个主题卡片；每日话语移入“记录与反馈”；AI 设置改为状态优先、字段按需显示、隐私摘要加详情面板；浏览器、APK 与真机回归结果见实施记录
- 2026-08-20：新增第 164 条意见（将“记录与反馈”拆为“记录设置”和“反馈设置”，每张卡片控制在约 2–3 个功能，降低单卡片高度与信息密度），仅记录
- 2026-08-20：新增第 165 条意见（增大 AI 设置卡片内部元素间距，并将“已配置”等状态小字移到“AI 设置”标题旁边，避免右侧独立显示造成局促），仅记录
- 2026-08-20：**#164/#165 实施完成（v3.35）**：将“记录与反馈”拆为“记录设置”和“反馈设置”；增加 AI 设置卡片内部间距，并将配置状态移到“AI 设置”标题旁内联显示；浏览器、APK 与真机回归结果见实施记录
- 2026-08-20：新增第 166 条意见（优化“记录设置”和“反馈设置”卡片内部的分割线；移除全宽内部横线，改用留白、字号和说明文字建立层级；不改变功能逻辑与配置数据），仅记录
- 2026-08-20：**#166 实施完成（v3.36）**：移除“记录设置 / 反馈设置”卡片内部全宽分割线，改用留白与文字层级建立分组；资源 v=128 / versionCode 57；浏览器、APK 与真机回归结果见实施记录
- 2026-08-20：新增第 167 条【Bug】（AI 设置新建/更换配置时仅显示 API Key，缺少模型编辑入口；自定义服务商的 Base URL/模型输入框未应用统一卡片样式），仅记录
- 2026-08-20：**#167 实施完成（v3.37）**：新建/更换时按服务商显示模型与 Base URL 编辑入口；统一 AI 文本输入框样式；资源 v=129 / versionCode 58；浏览器、APK 与真机回归结果见实施记录
- 2026-08-20：新增第 168 条意见（DeepSeek/OpenAI 与自定义服务商一样常显 Base URL 并预填官方默认地址；DeepSeek 新配置默认模型更新为 `deepseek-v4-flash`；旧模型只提示迁移、不自动覆盖），仅记录
- 2026-08-20：**#168/#169 实施完成（v3.38）**：内置服务商 Base URL 常显并自动填充默认地址；DeepSeek 新配置默认模型更新为 `deepseek-v4-flash`；旧模型迁移提示；API 已保存态仅优化状态点、按钮、间距和主题适配；资源 v=130 / versionCode 59；浏览器、APK 与真机回归结果见实施记录
- 2026-08-20：**#168 方案调整**：保存态隐藏 Base URL，与模型和 API Key 保持一致；仅在新建或点击“更换”进入编辑态时显示 Base URL；旧模型迁移提示仅在编辑态显示，避免保存态增加额外信息密度
- 2026-08-20：新增第 169 条意见（保持 AI API 已保存态现有“已保存 + 更换”结构与逻辑，仅优化状态点、按钮、间距、触摸区域和主题适配样式；不新增配置摘要、字段展示或编辑入口），仅记录
- 2026-08-20：**#168/#169 方案调整**：保存态不再重复显示“AI 已配置”或只显示“API 密钥已保存”；标题旁保留整体配置状态，保存态改为单行显示当前提供商与模型；密钥内容和完整 Base URL 仍隐藏，点击“更换”进入完整编辑态
- 2026-08-20：**#168/#169 实施完成（v3.39）**：保存态改为单行显示当前提供商与模型；标题旁唯一显示整体“已配置”并增加状态点；保存态隐藏 API Key、完整 Base URL、模型输入与旧模型迁移提示；点击“更换”恢复完整编辑态；资源 v=131 / versionCode 60；浏览器、APK 与真机回归通过
- 2026-08-20：新增第 170 条【Bug】（点击 AI 设置“更换”后自动聚焦 API Key 输入框，导致页面瞬移；已确认改为展开完整编辑态但不自动聚焦任何输入框），仅记录
- 2026-08-20：新增第 171 条意见（AI 设置卡片内部间距再次优化；加强主标题、副标题、配置摘要、字段标题和操作按钮之间的分层；“更换”按钮改名为“编辑”），仅记录
- 2026-08-20：**#170/#171 实施完成（v3.40）**：点击“编辑”后只展开完整编辑态，不再自动聚焦或滚动页面；重新拉开 AI 设置卡片标题、说明、字段和操作区间距并强化层级；资源 v=132 / versionCode 61；浏览器、APK 与真机回归通过
- 2026-08-20：新增第 172 条意见（记录设置增加可关闭的“计时通知”开关，并按方案 A 重排测试实况通知模块：移除下拉箭头、独立测试按钮、结构化结果区与开关状态联动），仅记录
- 2026-08-20：**#172 决策确认**：计时通知默认开启；系统通知权限未开启时保留 App 开关开启状态，在测试结果中提醒用户打开系统通知权限；相关默认值和权限提示纳入实施验收。
- 2026-08-20：**#172 实施完成（v3.40 内补丁，版本号不变）**：记录设置新增默认开启的计时通知开关；关闭时保留 App 内计时但跳过原生通知并即时停止已有通知；测试入口改为独立按钮，结果改为结构化状态行并区分浏览器、权限未开启、Android 16+ 提升和低版本降级；资源缓存参数更新至 v=133；浏览器、APK 与真机回归通过。
- 2026-08-20：**#173 实施完成（v3.40 内补丁，版本号不变）**：实况通知能力检测统一覆盖 Android Live Updates 与魅族 Flyme；任一路径可用时显示“实况通知已支持”；资源缓存参数更新至 v=134；浏览器回归通过，APK/真机结果见实施记录。
...
## 162. 【Bug】分段滑块背景层遮挡 tab 内容 ✅ 已修复（v3.33）

### 需求

用户反馈：“现在的分段滑块的滑块背景层，tab 的层级在下，tab 栏会被滑块背景层遮挡。”需要修复滑块背景与 tab/分段文字、图标的层级关系，确保内容始终可见且点击区域正常。

### 现状与根因定位

- `www/css/sheets.css` 中 `.seg-slide` 是绝对定位的滑块背景，`www/css/tabbar.css` 中 `.tab-slide` 是底部 tab 的滑块背景；两者都参与同一类“背景滑块 → 内容按钮”的层叠关系。
- 当前滑块背景没有显式的底层 `z-index` 和 `pointer-events: none`，内容层级依赖浏览器默认绘制顺序；液态玻璃的 `filter`、定位和局部 stacking context 会放大层级差异。
- `.tab` 虽设置了 `z-index: 1`，但 `.tab-pill` 只在部分主题下设置层级；分段控件也没有建立独立的层叠上下文，因此不同主题/设备 WebView 下可能出现背景盖住文字或图标的情况。
...
```text
容器：isolation: isolate
  滑块背景：z-index: 0，pointer-events: none
  tab/seg 内容：z-index: 1
  focus-visible/按压反馈：z-index: 2（仅需要时）
```

- `.seg-row` 与 `.tabbar` 建立独立 stacking context，避免滑块层与页面其他卡片或浮层互相影响。
- `.seg-slide`、`.tab-slide` 固定为 `z-index: 0` 且不接收指针事件；滑块只负责材质与位置，不参与交互命中。
- `.seg`、`.tab` 和 `.tab-pill` 统一显式设置内容层级；焦点轮廓和按压反馈不能被滑块裁切或覆盖。
- 不改变滑块的颜色、动画、液态玻璃滤镜和当前选中逻辑，只修复绘制顺序与点击穿透。
- 页面底部 tab 栏继续高于普通页面内容，但低于记录面板、对话框和全屏计时层；不通过无上限提高 z-index 解决局部遮挡。

### 实施清单（已完成）

1. ✅ `www/css/sheets.css`：为 `.seg-row` 增加 `isolation: isolate`；将 `.seg-slide` 固定为底层并设置 `pointer-events: none`。
2. ✅ `www/css/tabbar.css`：为 `.tabbar` 建立局部层叠上下文；将 `.tab-slide` 固定为 `z-index: 1`、不可命中；`.tab` 与 `.tab-pill` 固定在内容层；保留既有全局层级关系。
3. ✅ `www/css/glass.css`：将液态玻璃 `tabbar` 伪元素降到内容层以下，避免光带/边缘覆盖文字图标。
4. ✅ 浏览器回归：覆盖首页趋势 seg、记录页“就现在/补记”、我的页分段控件及底部三个 tab，检查文字、图标、焦点、按压和点击穿透。
5. ✅ 真机回归：魅族 WebView 验证液态玻璃、层级、点击与有效横向溢出。

### 实施决策

- 已统一采用 `z-index: 0 + pointer-events: none` 的滑块底层方案，避免不同控件继续出现层级差异。
- 保留 tabbar 相对于记录面板、对话框和全屏计时层的既有全局层级，仅修复容器内部绘制顺序。

### 验收要点

- 所有分段控件的文字和图标始终显示在滑块背景之上，选中态与未选中态均清晰。
- 滑块背景不拦截点击，点击 tab/seg 任意文字或空白区域都能正常切换。
- 焦点轮廓、按压反馈和液态玻璃滤镜不被裁切或覆盖。
- 底部 tab 栏仍能被记录面板、对话框和全屏计时页正确覆盖，不出现全局 z-index 回归。
- 浏览器与真机在主题、字体缩放和不同 WebView 下均无层级遮挡或控制台错误。

## 161. “我的”页控件语义与样式统一 ✅ 已实施（v3.33）
...
1. ✅ 审计 `www/index.html` 中“我的”页控件，确认 `seg`、`switch`、`row-btn` 和普通动作按钮的真实行为分类。
2. ✅ 保持 `www/js/app.js`、`www/js/ai.js`、`www/js/secure.js` 的事件绑定与配置、弹层、数据操作逻辑不变。
3. ✅ 在 `www/css/screens.css` 中将“我的”页 `row-btn` 右侧箭头旋转为右箭头，移除普通入口的下拉暗示；继续复用现有 `row-btn`、`seg`、`switch` 和按钮样式。
4. ✅ 不新增虚假的下拉 ARIA 语义；原生 `button` 保持动作入口语义，真实分段/开关继续沿用既有状态表达。
5. ✅ 完成浏览器与魅族 21 真机回归，覆盖点击区域、状态切换、液态玻璃层级、无横向溢出和大屏 WebView 绘制顺序。

### 实施决策

- 所有“我的”页现有 `row-btn` 入口统一使用右侧 chevron 表达进入/执行，不改变点击路径。
- 真正的单选项继续保留当前滑动分段样式，直接表达当前值，不增加点击层级。
- “测试连接”“导出数据”等既有动作保留原事件绑定和列表行点击路径；主动作按钮仍使用明确动作文案。
...
## 163. “我的”页整体信息架构与 AI 设置卡片精简 ✅ 已实施（v3.34）

### 需求

用户确认按推荐方案优化“我的”页，重点解决 AI 设置卡片中“AI 是可选增强功能”和 AI 数据隐私说明占用大面积空间、视觉重复的问题；同时将每日话语从 AI 设置移到记录相关的反馈区域，降低设置页的认知成本。

### 现状

- “我的”页当前由多个独立设置卡片组成，记录方式、记录提醒、正向反馈、实况通知等相关功能分散在不同卡片中。
- AI 设置卡片同时承担 AI 配置、AI 可选说明、隐私说明和每日话语开关，内容层级混杂，卡片高度偏大。
- Base URL、模型等自定义字段对普通 DeepSeek/OpenAI 用户并非必需，却默认占用空间。
- 已配置 API Key 的用户仍会看到完整输入表单，缺少“已配置”的紧凑状态反馈。

### 理解与设计（已确认方案）

#### 1. 页面信息架构

“我的”页保留 6 个主题卡片，按以下顺序组织：

1. **外观**：主题、玻璃效果、磨砂强度。
2. **记录与反馈**：记录方式、每日温和提醒、正向反馈、每日话语、实况通知。
3. **AI 设置**：服务商、API Key、连接测试、AI 数据范围。
4. **数据与隐私**：加密、本地数据操作、备份与恢复。
5. **危险操作**：清除全部数据、恢复演示数据。
6. **关于观己**：版本、隐私说明、本地存储说明。

不增加快捷设置区或折叠面板，继续复用现有卡片和列表行，减少重复入口。

#### 2. 每日话语迁移

- 将 `#aiDailyTipRow` 从 AI 设置卡片移动到“记录与反馈”卡片。
- 保留原有开关字段、缓存和触发逻辑。
- 行说明改为：`开启后：首页显示每日一句`。
- 补充一行：`配置 AI 后优先使用 AI 生成内容，未配置时使用本地话语`。
- 不改变 AI 每日话语与本地话语兜底的行为。
...
- 删除独立的大段“AI 是可选增强功能”说明。
- 删除卡片内的大段隐私说明，替换为一行摘要：`仅发送脱敏聚合统计 · 不上传单条记录`。
- 增加“了解数据范围”列表行，点击底部说明面板查看完整隐私边界：不发送单条记录、备注、附件，只发送次数、时段、标签比例等聚合统计。
- 已配置 API Key 时显示“已保存 / 更换”，默认隐藏输入框；未配置时显示输入框和保存按钮。
- Base URL、模型仅在选择“自定义”服务商时显示；老用户已有自定义配置时自动显示，不能丢失原值。
- “测试连接”保留为次级动作按钮，结果在按钮下方一行展示，不再使用额外说明段落。
...
- Switch：只用于开关。
- 右箭头列表行：只用于打开配置或详情。
- 明确动作按钮：用于测试连接、直接导出等立即执行动作。
- 不新增虚假下拉 ARIA 语义，不改变 #161/#162 已完成的箭头和层级修复。

### 实施清单（已完成）

1. ✅ `www/index.html`：调整“我的”页 6 个卡片的结构和顺序；移动 `aiDailyTipRow`；压缩 AI 说明；增加 AI 状态、隐私摘要和“了解数据范围”入口；保留现有 DOM ID。
2. ✅ `www/js/app.js` / `www/js/ai.js` / `www/js/ui-sheet.js`：复用现有配置存储；实现每日话语本地兜底、AI 状态渲染、已配置状态、自定义字段按需显示和隐私详情面板。
3. ✅ `www/css/screens.css`：统一卡片、列表行、状态副标题、次级动作按钮、详情入口和记录反馈子分组的间距与字体层级。
4. ✅ `www/css/glass.css`：沿用现有材质和 #162 层级修复，确认新卡片在液态玻璃模式下不产生覆盖。
5. ✅ 数据兼容：不改变 API Key、Base URL、模型、`aiDailyTip`、WebDAV、加密和备份字段；新增每日话语可见性字段默认开启以保持旧用户体验。
6. ✅ 版本与回归：升级至 v3.34，完成浏览器、APK、魅族 21 真机和 AI 无 Key/已配置/自定义/异常状态回归。

### 实施决策

- 已完成推荐方案：使用“记录与反馈”作为每日话语归属；AI 卡片采用状态优先、字段按需显示、隐私摘要加详情面板的结构。

### 验收要点

- “我的”页最终只保留 6 个主题卡片，记录相关功能集中在“记录与反馈”。
- 每日话语开关不再出现在 AI 设置卡片中，AI 未配置时仍能使用本地话语。
- AI 卡片不再显示两段大面积说明，未配置/已配置/异常状态清晰可见。
- DeepSeek/OpenAI 默认不显示 Base URL 和模型；自定义服务商及老用户配置正确显示。
- 已配置 API Key 时输入框默认收起，点击“更换”后可编辑；原有密钥和模型配置不丢失。
- “了解数据范围”可查看完整隐私说明，AI 请求仍只发送脱敏聚合统计。
- 浅色、深色、液态玻璃、大字体和真机窄屏下无横向溢出、无大面积空白、无控件遮挡。
- 所有现有设置、备份、加密、导入导出和危险操作路径保持可用。

---

## 164. “我的”页拆分“记录设置”与“反馈设置” ✅ 已实施（v3.35）

### 需求

当前 #163 实施后的“记录与反馈”卡片包含记录方式、记录提醒、正向反馈、每日话语和实况通知，单张卡片纵向占用空间过大，导致“我的”页整体视觉不协调。用户希望将其拆分，使每个板块平均承载约 2–3 个功能。

### 现状

- “记录与反馈”是当前“我的”页中内容最多、纵向高度最大的卡片。
- 记录方式、提醒、里程碑反馈、每日话语和实况通知虽然都属于设置，但使用场景并不相同。
- #163 已将每日话语从 AI 设置迁移到记录与反馈；本条在此基础上继续优化信息架构，不撤销该迁移。

### 理解与设计（已确认方案）

“我的”页由 6 个主题卡片调整为 7 个主题卡片，推荐顺序为：

1. **外观**：主题、玻璃效果、磨砂强度。
2. **记录设置**：记录方式、每日温和提醒、实况通知。
3. **反馈设置**：连续记录里程碑肯定、每日话语。
4. **AI 设置**：服务商、API Key、连接测试、AI 数据范围。
5. **数据与隐私**：加密、本地数据操作、备份与恢复。
6. **危险操作**：清除全部数据、恢复演示数据。
7. **关于观己**：版本、隐私说明、本地存储说明。

#### 1. 记录设置卡片

- 保留 `recordModeSeg`：计时记录 / 快速记录。
- 保留 `reminderSwitch`、`reminderTimeRow` 和 `reminderTime`，提醒时间仍沿用现有动态显示与原生时间选择交互。
- 将 `liveTestBtn` 和 `liveTestStatus` 放入记录设置卡片底部，作为记录过程相关的次级操作；不改变测试逻辑。
- 删除重复的长说明，只保留必要的短副标题或行内提示。

#### 2. 反馈设置卡片

- 保留 `positiveSwitch`：连续记录里程碑肯定。
- 保留 `aiDailyTipRow`、`aiDailyTipSwitch` 和 `aiDailyTipMigration`：每日话语继续属于反馈设置。
- 保留“配置 AI 后优先使用 AI，未配置时使用本地话语”的兜底文案与行为。
- 两个反馈开关采用统一列表行高和说明层级，不再使用完整独立模块的重复标题。

#### 3. 视觉与交互

- “我的”页卡片数量由 6 张变为 7 张，但每张卡片的内容密度更均衡。
- 两张新卡片沿用现有 `settings-card`、`settings-subtitle`、`switch-row`、`seg-row` 和 `row-btn`，不新增灰色内嵌背景或新的控件样式。
- 记录设置与反馈设置各自只保留一个主要分组标题，减少分割线和说明段落。
- 不新增折叠、“更多”或额外设置页；核心开关仍直接展示。
- 不改变任何配置键、事件绑定、每日话语本地兜底、提醒通知或实况通知行为。

### 实施清单

1. ✅ `www/index.html`：将 `recordFeedbackCard` 拆为 `recordSettingsCard` 与 `feedbackSettingsCard`，保持现有功能 DOM ID 不变，调整卡片顺序。
2. ✅ `www/css/screens.css`：为两张卡片统一标题、分组、行高、分割线和说明文字间距，避免拆分后出现新的大面积空白。
3. ✅ `www/js/ui-sheet.js`、`www/js/ai.js`：确认每日话语、提醒、里程碑反馈和实况通知的事件绑定不依赖旧卡片容器，数据流保持不变。
4. ✅ 更新版本号、资源缓存参数和 `CHANGELOG.md`，保留旧用户配置兼容。
5. ✅ 完成浏览器、液态玻璃、大字体和真机窄屏回归，覆盖 7 张卡片顺序、滚动节奏、提醒时间显示和每日话语开关。

### 待确认问题

- 已按确认方案实施：采用“记录设置 / 反馈设置”两个卡片名称和 7 卡片顺序。

### 验收要点

- “记录设置”只包含记录方式、提醒、实况通知三类功能。
- “反馈设置”只包含里程碑肯定和每日话语两类功能。
- 两张卡片高度明显低于当前“记录与反馈”单卡片，页面整体节奏更均衡。
- 每日话语未配置 AI 时仍显示本地话语，配置 AI 后仍优先使用 AI。
- 提醒开关、提醒时间、实况通知测试、里程碑反馈均保持原有行为和持久化。
- 旧用户配置、浅色/深色/玻璃/大字体和窄屏布局均无回归问题。

---

## 165. AI 设置卡片间距与状态标题排版优化 ✅ 已实施（v3.35）

### 需求

当前 AI 设置卡片内部各元素间距过小，提供商、API 密钥、测试连接和数据范围等控件挤在一起，整体显得局促。用户希望增加卡片内部呼吸感，并将“已配置”等状态小字放到“AI 设置”标题旁边，不再单独放在标题行右侧。

### 现状

- AI 卡片使用状态标题、可选增强摘要、提供商分段控件、API 密钥区域、测试连接和数据范围入口等多个层级。
- 现有标题行将 `#aiConfigStatus` 独立放在右侧，标题与状态被拉开，视觉上像两个不相关的元素。
- 部分标签、控件和次级说明之间的垂直间距不足，尤其是已配置状态下的密钥行、自定义服务商字段和测试连接区域。

### 理解与设计（草案）

#### 1. 状态并入标题

- 将 `#aiConfigStatus` 从标题行右侧独立区域移动到“AI 设置”标题之后，形成：`AI 设置 · 已配置`。
- `未配置`、`已配置`、`配置异常`三种状态继续保留颜色和语义，只改变布局位置。
- 状态使用小字号、轻量字重，不做胶囊按钮，不改变点击行为。
- 标题与状态在窄屏下允许自然换行，但不得产生横向溢出。

#### 2. 增加卡片内部呼吸感

- 调整 `.settings-card-heading`、`.ai-optional-summary`、`.field-label`、`.seg-row`、`.picker-row`、`.ai-key-saved-row`、测试连接区域和 `.ai-data-scope-btn` 之间的垂直间距。
- 标题区与摘要之间、摘要与提供商控件之间、API 密钥标签与输入行之间使用更明确的层级间距。
- 自定义服务商的 Base URL / 模型字段组之间增加间距，但不改变“仅自定义服务商显示”的逻辑。
- 已保存密钥区域与“更换”动作保持一组，不额外增加说明段落。
- 适度增加卡片上下内边距，避免只放大局部间距造成底部失衡。

#### 3. 保持既有视觉语言

- 继续复用现有 `settings-card`、`field-label`、`picker-row`、`btn-primary`、`btn-ghost` 和 `row-btn`。
- 不增加灰色内嵌背景、渐变或新的卡片材质。
- 保持 AI 数据范围摘要、详情弹层、API Key 兼容、自定义字段按需显示和每日话语逻辑不变。

### 实施清单

1. ✅ `www/index.html`：调整 AI 标题结构，让 `#aiConfigStatus` 成为标题后的内联状态元素，保留现有 ID。
2. ✅ `www/css/screens.css`：重新设定 AI 卡片各层级的间距、标题状态对齐、窄屏换行和上下内边距。
3. ✅ `www/js/ui-sheet.js`：确认 `renderAISettingsUI()` 继续只更新状态文本与 `data-state`，不改变配置和交互逻辑。
4. ✅ 完成浏览器与真机回归，覆盖未配置、已配置、自定义服务商、配置异常、液态玻璃、大字体和窄屏状态。

### 待确认问题

- 已按确认方案实施：采用“AI 设置 · 已配置”的标题内联形式，并按整体卡片比例增加间距。

### 验收要点

- “已配置”不再独立占据标题行右侧，而是紧邻“AI 设置”标题显示。
- 未配置、配置异常状态同样保持标题旁的内联位置和清晰可读性。
- 提供商、API 密钥、连接测试、数据范围之间不再显得拥挤，同时卡片底部不出现新的大面积空白。
- DeepSeek/OpenAI、自定义服务商和已保存密钥的显示逻辑保持不变。
- 浅色、深色、液态玻璃、大字体和真机窄屏无横向溢出、遮挡或状态错位。

---

## 166. “记录设置 / 反馈设置”卡片内部去分割线优化 ✅ 已实施（v3.36）

### 需求

当前“记录设置”和“反馈设置”两个卡片内部使用了多条全宽分割线。分割线将单个玻璃卡片切割成多个小区域，造成视觉噪音和层级割裂，整体不够协调。用户希望继续优化两个卡片的内部排版。

### 现状

- “记录设置”中，记录方式与提醒分组之间、实况通知操作上方存在全宽分割线。
- “反馈设置”中，两个反馈开关之间存在全宽分割线。
- 这些分割线同时配合多个小标题、顶部内边距和说明文字使用，导致卡片内部像多个嵌套模块。
- 当前功能、事件绑定、配置字段和数据逻辑均正常，本条只涉及视觉层级与间距。

### 理解与设计（已确认方案）

采用“去除全宽内部分割线，改用留白与层级区分”的方案：

#### 1. 记录设置卡片

- 保留“记录方式”和“提醒与通知”两个小标题。
- 移除两个分组之间的全宽 `border-top`。
- 移除“测试实况通知”操作上方的全宽分割线。
- 通过约 22–26px 的垂直留白区分两个功能组。
- “测试实况通知”作为提醒与通知组内的次级操作，使用较小字号、弱化颜色和现有行内说明表达层级。
- 不新增灰色内嵌背景、嵌套卡片或折叠面板。
...
- 保留“反馈偏好”分组标题。
- 移除两个反馈开关之间的全宽分割线。
- 两个开关行使用约 18–20px 的统一垂直间距。
- 通过标题、说明文字、开关位置和行高建立主次关系，不额外增加顶部内边距。

#### 3. 视觉与兼容边界

- 两张卡片只保留外层卡片边界，内部默认不使用全宽实线。
- 继续复用现有 `settings-card`、`settings-subtitle`、`switch-row`、`seg-row` 和 `row-btn` 样式。
- 保持浅色、深色、液态玻璃、大字体和窄屏下的现有材质与控件语义。
- 不改变记录方式、提醒、实况通知、里程碑反馈、每日话语的行为、配置键、持久化和老用户数据兼容。
- 如实测仍需要分隔，只允许使用短距离内嵌细线作为备选，不恢复全宽分割线。

### 实施清单（已完成）

1. ✅ `www/css/screens.css`：移除记录设置分组标题、实况通知次级操作和反馈开关之间的全宽 `border-top`；重设分组间距、开关行间距和说明文字节奏。
2. ✅ 复核 `www/index.html` 的现有层级，无需新增功能容器或修改交互结构。
3. ✅ 保持 `www/js/ui-sheet.js`、`www/js/ai.js` 及相关设置事件绑定不变，确认本条未引入数据逻辑改动。
4. ✅ 完成浏览器、浅色/深色/液态玻璃、大字体和真机窄屏回归，重点检查卡片高度、留白、行对齐、滚动节奏和横向溢出。

### 待确认问题

- 已完成：默认采用“无全宽内部分割线”的方案；短内嵌细线未启用。

### 验收要点

- “记录设置”和“反馈设置”卡片内部不再出现全宽分割线，卡片保持完整的玻璃面板感。
- 记录方式、提醒与通知、实况通知之间仍能通过留白和文字层级清晰区分。
- 两个反馈开关保持统一行高、间距、说明文字和开关对齐。
- 卡片不会因为删除分割线而产生新的大面积空白或信息拥挤。
- 所有设置行为、开关状态、提醒时间、每日话语本地兜底和旧用户配置保持不变。
- 浅色、深色、液态玻璃、大字体和真机窄屏无遮挡、横向溢出或布局错位。

---

## 167. 【Bug】AI 设置新建与更换配置缺少模型入口、输入框样式缺失 ✅ 已修复（v3.37）

### 需求

修复 AI 设置在新建和更换配置时只显示 API Key 输入框的问题，并恢复模型编辑入口；同时让自定义服务商的 Base URL、模型输入框使用与 AI 设置卡片一致的输入样式。

### 现状

- 当前 Base URL 和模型输入框都放在 `#aiCustomFields` 容器内。
- `renderAISettingsUI()` 仅在 `provider === 'custom'` 时显示该容器，因此新建或已配置的 DeepSeek/OpenAI 都只看到 API Key，无法修改模型名称。
- 点击“更换”只切换 `aiKeyEditing` 和 API Key 行的可见性，不会让模型字段进入编辑状态。
- CSS 只为 `.card > input[type="text"]` 提供卡片输入框样式；嵌套在 `#aiCustomFields` 中的输入框未命中该选择器，显示为浏览器默认样式。
- 现有 `provider / baseUrl / model / apiKey` 数据结构和默认值仍然存在，本问题主要是显示与编辑状态不完整。

### 理解与设计（已确认方案）

#### 1. 按状态显示配置项

| 状态 | DeepSeek / OpenAI | 自定义 |
| --- | --- | --- |
| 新建配置 | 模型 + API Key | Base URL + 模型 + API Key |
| 已保存状态 | 仅显示已配置状态 | 仅显示已配置状态 |
| 点击更换 | 模型 + API Key | Base URL + 模型 + API Key |

- DeepSeek/OpenAI 的 Base URL 继续使用默认值并隐藏，避免普通用户看到不必要字段。
- 模型名称在新建和编辑状态均可修改。
- 自定义服务商始终需要 Base URL 和模型输入。
- 模型继续使用文本输入，不使用固定下拉列表，以兼容不断变化的服务商模型和自定义模型。

#### 2. 统一输入框视觉

- 将模型、Base URL、API Key 等文本输入统一使用设置页输入框样式：卡片背景、细边框、圆角、内边距、主题色焦点态和当前字体变量。
- 不再依赖 `.card > input[type="text"]` 这种脆弱的层级选择器，改用可复用的语义类名。
- 保持浅色、深色、液态玻璃、大字体和窄屏下的视觉适配。

#### 3. 数据与兼容边界

- 保留现有 `provider / baseUrl / model / apiKey` 字段和各服务商独立配置。
- 保存时继续写入当前服务商配置；切换服务商不覆盖其他服务商的模型和密钥。
- 已有用户的 API Key、Base URL、模型和自定义配置不迁移、不丢失。

### 实施清单（已完成）

1. ✅ `www/index.html`：将模型字段从仅限自定义的容器中拆出；为模型和 Base URL 补充统一输入类名。
2. ✅ `www/js/ui-sheet.js`：扩展新建/编辑状态渲染逻辑；点击“更换”时显示模型，选择自定义时额外显示 Base URL。
3. ✅ `www/css/screens.css`：为嵌套 AI 输入框补齐卡片材质、边框、圆角、间距和焦点样式。
4. ✅ 保持 `www/js/ai.js` 的默认服务商、配置存储和 `saveAIConfig()` 数据流兼容。
5. ✅ 回归新建 DeepSeek/OpenAI、自定义配置、已配置点击更换、切换服务商、保存后重启和窄屏布局。

### 待确认问题

- 已完成：默认服务商显示模型但隐藏 Base URL；自定义服务商显示 Base URL 与模型；模型使用文本输入而非固定下拉列表。

### 验收要点

- 新建 DeepSeek/OpenAI 时可看到并编辑模型和 API Key。
- 新建自定义服务商时可看到并编辑 Base URL、模型和 API Key。
- 已配置 DeepSeek/OpenAI 点击“更换”后可修改模型和 API Key。
- 已配置自定义服务商点击“更换”后可修改 Base URL、模型和 API Key。
- 所有输入框在不同主题、玻璃效果、大字体和窄屏下样式统一、无横向溢出。
- 保存后刷新或重新打开 App，当前服务商的模型、Base URL 和 API Key 均正确保留。

---

## 168. 统一内置服务商 Base URL 与 DeepSeek 默认模型 ✅ 已实施（v3.39）

### 需求

让 DeepSeek、OpenAI 与自定义服务商保持一致：新建或编辑时显示 Base URL 并自动填入官方默认地址；保存态与 API Key、模型名称一样隐藏 Base URL。同时更新 DeepSeek 新配置的默认模型，降低旧模型弃用带来的连接风险。

### 现状

- 当前 DeepSeek 与 OpenAI 已有默认 Base URL，但内置服务商在界面中隐藏；自定义服务商才显示 Base URL 输入框。
- 当前 DeepSeek 新配置默认使用 `deepseek-chat`，官方已将其列入弃用模型，推荐使用 `deepseek-v4-flash` 或 `deepseek-v4-pro`。
- 请求层会对 Base URL 去除末尾斜杠后拼接 `/chat/completions`；DeepSeek 默认地址继续使用 `https://api.deepseek.com`，不额外添加 `/v1`。

### 理解与设计（已确认方案）

#### 1. Base URL 统一显示

| 服务商 | Base URL 默认值 | 显示策略 |
| --- | --- | --- |
| DeepSeek | `https://api.deepseek.com` | 新建/编辑时显示；保存态隐藏 |
| OpenAI | `https://api.openai.com/v1` | 新建/编辑时显示；保存态隐藏 |
| 自定义 | 空 | 新建/编辑时显示；保存态隐藏 |

- 内置服务商切换后自动填入对应默认地址。
- 新建和编辑状态显示 Base URL 并预填默认地址；保存态隐藏，与模型和 API Key 保持一致；点击“更换”后恢复显示。
- 模型字段继续沿用 #167 的文本输入设计；不改为固定下拉列表。

#### 1.1 保存态收敛（后续调整，待实施）

- 保存态不重复显示“AI 已配置”；整体状态仅由标题旁的“AI 设置 · 已配置”表达。
- 保存态使用一行轻量摘要显示当前提供商和模型，例如 `DeepSeek · deepseek-v4-flash`；不显示 API Key 内容或完整 Base URL。
- 新建或点击“更换”进入编辑态后，恢复 Base URL、模型和 API Key 输入。
- Base URL 仍按服务商保存，不改变 `provider / baseUrl / model / apiKey` 数据结构。
- 旧模型迁移提示仅在编辑态显示；保存态不额外显示提示，保持卡片紧凑。

#### 2. DeepSeek 默认模型更新

- 新建 DeepSeek 配置默认模型改为 `deepseek-v4-flash`。
- `deepseek-v4-pro` 作为可手动填写的高级模型保留。
- 老用户已保存的 `deepseek-chat` 不自动覆盖。
- 检测到旧模型时显示非阻断式迁移提示，用户可自行改为新模型。
- OpenAI 当前模型默认值本次不调整，另行评估兼容性。

#### 3. 数据兼容边界

- 保留现有 `provider / baseUrl / model / apiKey` 字段及各服务商独立配置。
- 已有用户的 Base URL、模型和 API Key 不丢失、不静默替换。
- 新默认值只作用于没有保存配置的新建流程。

### 实施清单

1. ✅ `www/js/ai.js`：更新 DeepSeek 默认模型及服务商默认配置，并补齐缺失的内置 Base URL。
2. ✅ `www/index.html`：调整 Base URL 字段、模型占位文案和迁移提示容器。
3. ✅ `www/js/ui-sheet.js`：完成 DeepSeek/OpenAI/自定义的 Base URL 字段补齐与编辑态切换；保存态最终显示规则按“后续调整清单”收敛为隐藏。
4. ✅ 增加旧 `deepseek-chat` / `deepseek-reasoner` 的非阻断迁移提示，不自动修改老用户配置。
5. ✅ 回归 DeepSeek、OpenAI、自定义服务商的切换、编辑、保存、重启和窄屏布局。

### 后续调整清单（已完成）

1. ✅ 将 `www/js/ui-sheet.js` 的保存态 Base URL 显示规则改为隐藏；新建/编辑态保持显示。
2. ✅ 将保存态改为单行显示当前提供商与模型；标题旁保留整体“已配置”状态，不重复显示“AI 已配置”。
3. ✅ 将旧模型迁移提示限制为编辑态显示，保存态不增加额外提示。
4. ✅ 保持 #169 已完成的状态点、“更换”按钮、主题适配与现有配置数据兼容。

### 待确认问题

- 已确认：保存态隐藏 Base URL，与模型和 API Key 保持一致。
- 已确认：保存态显示当前提供商与模型摘要，不再重复显示“AI 已配置”。
- 已确认：旧模型迁移提示仅在编辑态显示。
- 已完成：上述显示规则已实现，并通过浏览器、APK 与真机回归。

### 验收要点

- DeepSeek、OpenAI、自定义服务商在新建/编辑时显示 Base URL，保存态隐藏；内置服务商自动填入正确默认值。
- 保存态一行显示当前提供商与模型；“AI 设置 · 已配置”只在标题旁出现一次；API Key 内容和完整 Base URL 不显示。
- Base URL 末尾带不带斜杠都不会造成重复路径。
- 新建 DeepSeek 配置默认显示 `deepseek-v4-flash`。
- 老用户的 `deepseek-chat` 配置保持不变；进入编辑态时能看到迁移提示，保存态不显示。
- 切换服务商、保存、重新打开 App 后，各自的 Base URL、模型和 API Key 正确保留。
- 浅色、深色、玻璃效果、大字体和窄屏下无横向溢出。

---

## 169. AI API 已保存态视觉样式优化 ✅ 已实施（v3.39）

### 需求

当前 AI API 已保存态的内容和操作逻辑已经确定，但“已保存 + 更换”这一行视觉较简陋且信息不足；需要在不展开完整表单的前提下，用一行摘要说明当前提供商与模型，同时提升状态表达、按钮质感和行内间距。

### 现状

- 已保存态仅显示“已保存”和“更换”，Base URL、模型和 API Key 编辑行按现有逻辑隐藏；用户无法快速确认当前提供商与模型。
- `ai-key-saved-row` 当前使用基础横向布局，状态文字与更换按钮的层级和完成感不足。
- #166 已移除设置卡片内部全宽分割线，本条继续使用留白和文字层级，不恢复分割线或灰色内嵌背景。

### 理解与设计（已确认方案）

1. **保持收纳，但补充最小配置摘要**
   - 标题旁的“AI 设置 · 已配置”作为唯一的整体配置状态，不在保存态重复显示。
   - 保存态左侧显示当前提供商与模型，例如 `DeepSeek · deepseek-v4-flash`；右侧继续保留“更换”。
   - 不显示 API Key 内容或完整 Base URL；点击“更换”后恢复完整编辑字段。
   - 不改变提供商切换、保存、测试连接和 API Key 存储逻辑。

2. **状态样式优化**
   - “已保存”使用清晰的中等字重和主题次级文字颜色。
   - 可增加低调的绿色状态点，与标题旁“已配置”状态呼应，但不使用大面积绿色。
   - 状态行保持透明、轻量，不增加灰色背景块。

3. **更换按钮优化**
   - 保留“更换”文字，不增加图标。
   - 使用轻量描边胶囊样式，降低普通灰色按钮的笨重感。
   - 保留现有 hover、focus-visible 和按压缩放反馈。
   - 触摸目标保持约 40–44px，兼顾真机操作和大字体模式。

4. **间距与主题适配**
   - 调整状态行左右间距、垂直居中和行高，让内容更有呼吸感。
   - 继续使用 `--line`、`--ink-2`、`--ink-3`、`--sage` 等主题变量。
   - 浅色、深色、液态玻璃、大字体和窄屏保持一致，不产生横向溢出。

### 实施清单

1. ✅ `www/css/screens.css`：优化 `.ai-key-saved-row`、状态文字和“更换”按钮的布局、尺寸、颜色与交互态。
2. ✅ 使用局部规则适配液态玻璃、深色和窄屏，不改变其他页面按钮。
3. ✅ 未修改 API 配置数据流；#168 的必要字段调整仅服务于 Base URL 常显功能。

### 后续调整清单（与 #168 联动，已完成）

1. ✅ `www/js/ui-sheet.js`：保存态渲染当前提供商与模型摘要；编辑态继续显示完整 Base URL、模型和 API Key 输入。
2. ✅ `www/index.html`：保留 API 密钥字段的可访问标签，保存态改为与提供商/模型摘要同一行或同一紧凑区域。
3. ✅ `www/css/screens.css`：控制摘要单行布局、模型过长省略、状态点与“更换”按钮的间距，兼容大字体、玻璃和窄屏。
4. ✅ 保持标题旁“已配置”只出现一次，不新增重复的“AI 已配置”文案。

### 待确认问题

- 已完成：原保存态视觉样式浏览器、APK 与真机回归通过。
- 已完成：轻量配置摘要已实现，并通过浏览器、APK 与真机回归。

### 验收要点

- 标题旁仅显示一次整体“AI 设置 · 已配置”；保存态一行显示当前提供商与模型，并保留“更换”。
- API Key 内容和完整 Base URL 不显示，点击“更换”后可进入完整编辑态。
- 状态点、文字层级和按钮样式与 AI 设置卡片整体风格协调。
- “更换”按钮在浅色、深色、玻璃效果和大字体模式下均保持清晰可点。
- 不恢复内部全宽分割线，不新增灰色内嵌背景，不影响测试连接和数据范围区域。
- 未配置态、已保存态、点击更换后的编辑态均无布局跳动或横向溢出。

---

## 170. 【Bug】AI 设置点击“更换”后页面自动跳到 API Key 输入框 ✅ 已修复（v3.40）

### 需求

点击 AI 设置保存态的“更换”按钮后，展开完整编辑态，但不自动聚焦 API Key 或其他任何输入框，避免页面瞬移，保持当前设置区域的视觉连续性。

### 现状

- 当前点击“更换”会展开 Base URL、模型和 API Key 等编辑字段。
- 同时自动将焦点放到 API Key 输入框，浏览器或 WebView 会为了保证输入框可见而滚动页面。
- 用户点击的是卡片内的操作，但页面视口会突然跳到输入框位置；在“我的”页较长、大字号或窄屏下更明显。

### 理解与设计（已确认方案）

- 保留“更换”按钮展开完整编辑态的行为。
- 移除“更换”操作后的自动聚焦行为，不主动聚焦 API Key、Base URL、模型或其他输入框。
- 用户可按自己的操作顺序手动点击需要编辑的字段；现有输入、保存、测试连接逻辑不改变。
- 本条只修复页面跳转和焦点策略，不改变 API Key 隐藏、配置摘要或编辑态字段显隐规则。

### 实施清单

1. ✅ `www/js/ui-sheet.js`：调整 `apiKeyChange` 点击处理，展开编辑态后不再调用输入框 `focus()`。
2. ✅ 检查其他编辑路径和初始化逻辑，确认不会间接触发自动滚动或自动聚焦。
3. ✅ 在窄屏、大字号、深色/玻璃效果和真机键盘环境下回归点击位置与页面滚动状态。
...
- 点击“更换”后编辑字段完整展开，页面不会瞬移或自动滚动到 API Key 输入框。
- 点击后没有任何输入框处于自动聚焦状态；用户手动点击输入框后仍可正常输入。
- Base URL、模型、API Key 的编辑、保存和测试连接流程不受影响。
- 窄屏、大字号、深色/玻璃效果和真机键盘场景无布局跳动或横向溢出。

---

## 171. AI 设置卡片内部间距与操作文案继续优化 ✅ 已实施（v3.40）

### 需求

优化 AI 设置卡片内部的空间关系与信息层级，解决修改后卡片内容再次变得局促、主标题与副标题及各级标题之间分层不明显的问题；同时将保存态操作按钮“更换”改名为“编辑”。

### 现状

- 当前 AI 设置卡片的各元素间距偏小，主标题、副标题、配置摘要、字段标题和操作区域之间缺少足够的呼吸空间。
- 信息层级不够明显，用户需要依靠文字样式而不是间距快速区分标题、说明和操作。
- 保存态按钮“更换”未能准确表达其实际行为：进入配置编辑态。

### 理解与设计（已确认方案）

- 增大卡片内部各元素之间的垂直间距，优先调整标题组、配置摘要、编辑字段组和操作区之间的间隔。
- 通过间距、字号、字重和行高强化主标题、副标题、字段标题及辅助说明的层级，不引入新的功能模块。
- 保存态操作按钮文案从“更换”改为“编辑”，点击后仍展开完整编辑态。
- 保持现有 API 设置逻辑、保存态隐藏规则、编辑态字段和“点击后不自动聚焦”的 #170 方案不变。

### 实施清单

1. ✅ `www/css/screens.css`：重新调整 AI 设置卡片标题组、摘要行、字段组和操作行的间距、字号、字重与行高。
2. ✅ `www/index.html`：将保存态操作按钮文案改为“编辑”，保留现有按钮结构与可访问语义。
3. ✅ 检查浅色、深色、玻璃效果、大字号和窄屏下的卡片高度、换行与横向溢出。
4. ✅ 与 #170 联动验证：点击“编辑”后仅展开编辑态，不自动聚焦任何输入框。
...
- 主标题、副标题、配置摘要、字段标题、辅助说明和操作按钮之间具有清晰、稳定的分层间距。
- 卡片内部不再显得拥挤，且不会因间距调整产生不必要的大面积空白。
- 保存态按钮显示为“编辑”，点击后可正常进入完整编辑态。
- 点击“编辑”后页面不瞬移、不自动滚动、不自动聚焦输入框。
- 浅色、深色、玻璃效果、大字号、窄屏和真机键盘场景无横向溢出或布局跳动。

---

## 172. 记录设置：计时通知开关与测试实况通知布局优化 ✅ 已实施（v3.40 内补丁，版本号不变）

### 需求

当前“记录设置 → 提醒与通知”只有“测试实况通知”入口，没有应用内的计时通知开关。用户无法在观己内部选择是否在计时期间显示锁屏/通知栏通知；同时现有测试入口使用列表行和下拉箭头样式，测试结果以多行文本直接堆叠，导致卡片层级和高度不稳定。

用户确认采用方案 A：将“计时通知”开关与测试实况通知重排为一个纵向设置模块。

### 现状

- `www/index.html` 的 `liveTestBtn` 使用 `row-btn`，右侧箭头会让用户误认为这是下拉菜单或进入下一层页面。
- `www/js/ui-timer.js` 的 `notifyStartTimer()` 在开始计时后无条件调用 `TimerLiveUpdate.startTimer()`；当前没有应用级启用状态。
- `www/js/ui-sheet.js` 的 `testLiveUpdate()` 将系统、权限和支持状态拼接为 `<br>` 多行文本，默认说明和测试结果也没有清晰区分。
- Android 系统通知权限与 App 内开关是两层不同的控制，不能互相替代。

### 理解与设计（已确认方案 A）

1. **增加应用内“计时通知”开关**
   - 位于“记录设置 → 提醒与通知”分组内。
   - 标题使用“计时通知”，避免把 Android 16 的“实况通知”误解为所有系统版本都具备的能力。
   - 说明文案为“计时中在锁屏和通知栏显示，可随时关闭”。
   - Android 16+ 的提升能力、低版本普通通知和系统权限状态在测试结果中说明。

2. **测试模块采用纵向结构**
   - 保留“计时通知”标题、说明和右侧开关。
   - 下方放置独立的“测试通知”按钮，不再使用 `row-btn` 和下拉箭头。
   - 测试前默认只显示简短说明，不展开结果区。
   - 点击测试后展开结构化结果区，以“项目 / 状态”对应显示系统版本、通知权限和显示方式。
   - 测试中按钮显示“测试中…”并暂时禁用；测试完成后显示可用状态。

3. **关闭开关后的行为**
   - App 内计时继续正常工作，不影响结束、继续计时、保存和放弃流程。
   - 不调用观己的计时通知启动/停止路径，不主动创建计时通知。
   - 测试按钮置灰并提示“开启计时通知后可测试”。
   - Android 系统通知权限仍作为独立状态处理；关闭 App 开关不承诺屏蔽系统所有前台服务提示。

4. **状态与视觉层级**
   - 去除测试入口右侧箭头，避免操作语义误导。
   - 结果区使用紧凑状态行、状态点和一条轻分隔线，不使用大面积灰色内嵌背景。
   - 计时通知开关、测试按钮和结果区在浅色、深色、玻璃效果、大字体和窄屏下保持同一层级关系。
   - 不改变 Android 原生通知的构建、15 秒测试时长、权限请求和实况通知提升逻辑。

5. **配置兼容**
   - 新增本地持久化设置，例如 `guanji_timer_notification_enabled`。
   - 新用户和已有用户默认开启计时通知，保持当前行为不变；用户关闭后持久化保存关闭状态。
   - 快速记录、补记和非计时流程不受影响。

### 实施清单

1. ✅ `www/index.html`：在“提醒与通知”中增加“计时通知”开关，按方案 A 重排测试模块，移除下拉箭头。
2. ✅ `www/js/ui-timer.js`：读取持久化开关；关闭时跳过 `TimerLiveUpdate.startTimer()` / `stopTimer()`，保留 App 内计时状态机；已有通知在关闭时强制停止。
3. ✅ `www/js/ui-sheet.js`：将测试状态从 `<br>` 文本改为结构化结果渲染，处理未测试、测试中、已支持、普通通知降级和权限未开启状态。
4. ✅ 同等设置存储入口：使用 `guanji_timer_notification_enabled` 持久化；缺少旧值时默认开启，兼容已有用户。
5. ✅ `www/css/screens.css`：实现纵向模块、独立测试按钮、状态行、禁用态和主题适配。
6. ✅ 回归直接记录、补记、计时记录、继续计时、通知结束/取消、权限拒绝、关闭开关和重新打开 App 等路径；浏览器与真机专项验证通过。

### 实施结果

- `www/index.html` 的资源查询参数由 `v=132` 更新为 `v=133`，仅用于 WebView 缓存刷新；`versionCode 61`、`versionName 3.40`、关于页 `v3.40` 均保持不变。
- 浏览器 390×844：默认开启、关闭持久化、测试按钮禁用/恢复、浏览器环境结构化结果、无横向溢出和无控制台错误。
- 真机 `461QYFDN226NF`：安装 APK 成功；计时通知开启时 `dumpsys notification` 可见 `com.guanji.app` 的 `4101` 常驻通知，关闭时 App 内计时仍运行且不产生该通知；CDP UI 回归通过。

### 实施决策

- 计时通知默认开启，兼容新用户和已有用户的当前行为。
- 系统通知权限未开启时，App 内“计时通知”开关仍保持开启；测试结果区明确显示“需要系统通知权限”，并提醒用户前往系统设置打开权限。

### 验收要点

- 记录设置中存在独立的“计时通知”开关，状态可保存并在重启 App 后恢复。
- 关闭开关后，App 内计时、结束、继续计时和保存流程正常，且不主动创建观己计时通知。
- 测试入口不再显示下拉箭头；按钮文案为“测试通知”，关闭状态下不可误触。
- 测试结果以结构化状态行显示，不再出现多段文本堆叠、卡片突然撑高或布局跳动。
- Android 16、低版本 Android、通知权限未开启和浏览器预览环境均有清晰状态提示。
- 浅色、深色、玻璃效果、大字体、窄屏和真机键盘环境无横向溢出或触摸区域异常。

---

## 173. 【Bug】实况通知能力检测未识别魅族 Flyme 支持 ✅ 已实施（v3.40 内补丁，版本号不变）

### 需求

当前 App 已同时支持魅族 Flyme 实况通知和 Android Live Updates，但测试通知的能力检测只判断 Android Live Updates。魅族真机实际可以显示实况通知，点击“测试通知”却被判定为“不支持”，需要修正检测结果。

### 现状

- 原生 `TimerLiveUpdate` 已存在魅族实况通知路径和 Android Live Updates 路径。
- 测试结果主要依据 Android Live Updates 的 `supported` 状态判断，未将魅族 Flyme 能力纳入统一支持判断。
- 该问题是能力检测与真实通知能力不一致，不需要重新设计当前记录设置 UI。

### 理解与设计（已确认）

- 保持当前测试模块的 UI 布局、按钮和交互流程不变，只修正能力检测逻辑。
- 检测同时覆盖 Android Live Updates 与魅族 Flyme 实况通知能力。
- 只要任一实况通知路径可用，测试结果统一显示“实况通知已支持”。
- 不在界面中展开或并列展示“魅族 / Android”能力来源；两者均不可用时才显示普通通知降级提示。
- 系统通知权限、计时通知开关和现有 15 秒测试流程保持不变。

### 实施清单

1. 检查 `TimerLiveUpdate` 原生插件的魅族能力判断、Android Live Updates 判断及返回给 WebView 的状态字段。
2. 将 JS 测试结果改为基于“任一实况通知能力可用”的统一判断，而不是只读取 Android `supported`。
3. 保留权限未开启、普通通知降级和浏览器预览环境的现有提示逻辑。
4. 在魅族真机与 Android Live Updates 设备上分别回归，确认实际支持时统一显示“实况通知已支持”。

### 实施结果（2026-08-20）

- `android/app/src/main/java/com/guanji/app/TimerLiveUpdatePlugin.kt`：新增 `flymeSupported`、`androidLiveUpdateSupported` 双路径状态；`supported` 改为两者任一可用，且共同受通知权限约束。
- `www/js/ui-sheet.js`：显示方式统一为“实况通知已支持”；测试说明不再只写 Android 16+，保留普通通知降级提示。
- `www/index.html`：同步更新测试说明，并将 WebView 资源缓存参数由 `v=133` 更新为 `v=134`；`versionCode`、`versionName`、关于页版本号保持不变。

### 验证结果

- `node --check www/js/ui-sheet.js`：通过。
- 浏览器回归：浏览器预览仍提示需 Android 真机；原生状态分支按统一 `supported` 字段渲染；无横向溢出与控制台错误。
- APK 构建与真机验证：见 `实施v3.40_#173_2026-08-20.md`；重点核验魅族设备显示“实况通知已支持”，Android Live Updates 路径保持兼容。
...
- 魅族真机实际支持实况通知时，测试结果显示“实况通知已支持”，不再误报“不支持”。
- Android Live Updates 支持设备仍显示“实况通知已支持”。
- 两种实况通知能力均不可用时，才显示普通通知降级提示。
- 当前测试模块 UI、按钮、权限处理、计时通知开关和 15 秒测试流程不发生回归。
- 浏览器预览环境仍明确提示需要 Android 真机测试。
